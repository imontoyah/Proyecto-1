...
private boolean splitArrayAux(int start,int[] nums,int targetA, int targetB) {
 if(start >= nums.length){ // C0.1 = 5 
     ...
     return splitArrayAux(start+1 ,nums, targetA+ x, targetB) || splitArrayAux(start+1 ,nums, targetA,x+ targetB); // T(n) = 2*T(n-1) +C2
     ...
     if(target == 0 && start == nums.length ) 
          return true;                           //C1 = 6 
           ...
           return false;                           // C2 = 10
            ...
            return groupSumClump(start +1 ,nums, target-nums[start]) || groupSumClump(start +1 ,nums, target); // C3 = 8      T(n) = 2*T(n-1) + C3
                                                                                                                //            T(n)=C3(2^n-1) +C1*2^(n-1)  
    
           
       else if(nums[start] == nums[start+1]  && -start+nums.length-1 >= 1)                                     
            return groupSumClump( start+2, nums,  target-2*nums[start])||groupSumClump( start+2, nums,target); //c4 = 16      T(n) = 2*T(n-2) + C4
                                                                                                               //             T(n)=2^(1/2)*(C2(-1)^(n)+ C1) - c4
             
      else 
         return groupSumClump(start +1 ,nums, target-nums[start]) || groupSumClump(start +1 ,nums, target);   // C5 = 6       T(n) = 2*T(n-1) + C_5 
