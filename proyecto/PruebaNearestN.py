...
# newH is the height(Altura) of the new image; newW is the width(Ancho) of the new image
def NN_interpolation(img,newH,newW):
    ...
    for i in range(newH-1):
        for j in range(newW-1):
            oldx = round(i*(oldH/newH)) #represent the zoom ratio of the height and width
            oldy = round(j*(oldW/newW))
            newimg[i,j] = img[oldx,oldy]
    return newimg

#https://www.programmersought.com/article/36124682709/#:~:text=The%20nearest%20neighbor%20interpolation%20method%20adds%20the%20value%20of%20the,significantly%2C%20often%20containing%20jagged%20edges.
...
#Grafico
rows, colums = original_image.shape
rowsF, columsF = img_descompress.shape
plt.xlabel('Pixels column')
plt.ylabel('Color representation(Gray Scale)')
plt.plot(range(0,rows), original_image[:,0],'r')
plt.plot(range(0,rows),img_descompress[:,0],'b')
plt.show()